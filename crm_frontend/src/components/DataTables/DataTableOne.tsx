...
const DataTableOne = ({results, columns, tabs}: DataTableProps) => {
  const data = useMemo(() => results, [results]);
  const defaultColumn = useMemo(() => {
    return {
      Filter: ColumnFilter as React.FC<FilterProps<any>>,
    };
  }, []);
  ...
  return (
    <section className="data-table-common rounded-sm border border-stroke bg-white py-4 shadow-default dark:border-strokedark dark:bg-boxdark">
      <div className="flex justify-between px-8 pb-4">
        { tabs && <TabOne tabs={tabs} handleTabClick={handleTabClick}/> }
        <div className="w-100">
          <input
            type="text"
            value={globalFilter}
            ...
            return (
              ...
              >
                {row.cells.map((cell, key) => {
                  return (
                    <td {...cell.getCellProps()} key={row.index + key}>
                      {cell.render("Cell")}
                    </td>
                  );
                })}
